...
bool change = true; //크기 변화에 사용될 변수
float scale_v; //크기 값 저장

mat3 t_move = (new float[3][3]{ //이동 행렬 변수
    {1, 0, 0},
    {0, 1, 0},
    {0.001f, 0, 1} });

mat3 r_rotate = (new float[3][3]{ //회전 행렬 변수
    {(float)cos((PI / 180) * 1) , -(float)sin((PI / 180) * 1), 0},
    {(float)sin((PI / 180) * 1), (float)cos((PI / 180) * 1), 0},
    {0, 0, 1} });

mat3 u_scale = (new float[3][3]{ //크기 커지는 행렬 변수
    {1.01f, 0, 0},
    {0, 1.01f, 0},
    {0, 0, 1} });

mat3 d_scale = (new float[3][3]{ //크기 작아지는 행렬 변수
    {0.99f, 0, 0},
    {0, 0.99f, 0},
    {0, 0, 1} });
//////////////////////////////////////////////////////////////////////////////////////////
...
void Update()
{
    while (!glfwWindowShouldClose(window))
    {
        ...
        transform.translate = transform.translate * t_move; //이동
        transform.rotation = transform.rotation * r_rotate; //회전
        if (change) {
            transform.scale = transform.scale * u_scale; //크기 확대
            scale_v += 0.01f;
            if (1 + scale_v >= 1.3f) {
                change = false;
            }
        ...
        if (!change) {
            scale_v -= 0.01f;
            transform.scale = transform.scale * d_scale; //크기 축소
            if (1 + scale_v <= 0.7f) {
                change = true;
            }
        }
